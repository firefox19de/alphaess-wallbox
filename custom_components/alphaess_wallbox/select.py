...
class AlphaESSModeSelect(CoordinatorEntity, SelectEntity):
    ...
    def current_option(self) -> str | None:
        ...
        return REVERSE_MODE_MAP.get(
            self.coordinator.data.get("chargeMode"), self._attr_current_option
        )

    async def async_select_option(self, option: str) -> None:
        mode_code = MODE_MAP.get(option, 4)
        if self.coordinator.data is not None:
            self.coordinator.data["chargeMode"] = mode_code
        self._attr_current_option = option
        self.async_write_ha_state()
        success = await self.coordinator.client.async_set_ev_charge_mode(mode_code)
        if success:
...
class AlphaESSPhaseSelect(CoordinatorEntity, SelectEntity):
    ...
    def current_option(self) -> str | None:
        ...
        return REVERSE_PHASE_MAP.get(
            self.coordinator.data.get("obcPhase"), self._attr_current_option
        )

    async def async_select_option(self, option: str) -> None:
        phase_code = PHASE_MAP.get(option, 3)
        if self.coordinator.data is not None:
            self.coordinator.data["obcPhase"] = phase_code
        self._attr_current_option = option
        self.async_write_ha_state()
        success = await self.coordinator.client.async_set_ev_phases(phase_code)
        if success:
            await self.coordinator.async_request_refresh()
        else:
